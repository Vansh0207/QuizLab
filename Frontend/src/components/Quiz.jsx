...
const QuizPage = () => {
    ...
    const fetchQuestions = async () => {
      try {
        const response = await axios.get(
          `${BASE_URL}/api/question/${id}/questions`
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/api/video/${id}/score`, {
        videoId: id,
      ...
      {!submitted ? (
        ...
        <div className="text-center py-8 px-4 max-w-2xl mx-auto bg-white shadow-lg rounded-lg">
          ...
          <div className="mt-6 flex items-center justify-center space-x-4">
            <div className="w-32 h-32">{getCircleScore()}</div>
          </div>
              ...
              {questions.map((question, index) => (
                <li key={index} className="flex flex-col space-y-2 mb-4">
                  ...
                  <div className="text-sm text-gray-600">
                    <strong className="text-gray-900">Correct Answer:</strong>{" "}
