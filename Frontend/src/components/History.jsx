...
const HistoryPage = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        const response = await axios.get(
          `${config.NODE_BACKEND}/api/user/userData/${user._id}`
        );
        console.log("API Response Data:", response.data);

        setHistory(
          Array.isArray(response.data.user.videos)
  ...
  const handleDelete = async (videoId) => {
    try {
      ...
      const response = await axios.delete(
        `${config.NODE_BACKEND}/api/video/delete/${videoId}`,
        {
  ...
  const getCircleScore = () => {
    const percentage =
      (selectedVideo.score / selectedVideo.questions.length) * 100;
  ...
  const fetchVideoDetails = async () => {
    try {
      const response = await axios.get(
        `${config.NODE_BACKEND}/api/video/summary/${selectedVideo._id}`,
        {
  ...
  const handleGenerateQuiz = async () => {
    try {
      ...
      const saveQuizResponse = await axios.post(
        `${config.NODE_BACKEND}/api/question/saveQuestion`,
  ...
  return (
    <div className="max-w-5xl min-h-[80vh] mx-auto p-6">
      ...
      <div className="mb-6 p-4 flex items-center justify-between bg-gray-200 rounded-lg text-center text-xl font-semibold">
        ...
        <Link to={"https://cartiify.vercel.app/"} target="_blank">
          <button className="bg-blue-500 text-white py-1.5 px-2.5 text-sm rounded-md shadow hover:bg-blue-600 transition cursor-pointer">
            Reedem Now
          </button>
        </Link>
      </div>
          ...
          {history.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {history.map((video) => (
                <div
                  key={video._id}
                  className={`relative border rounded-lg p-4 shadow hover:shadow-2xl cursor-pointer transition ${
