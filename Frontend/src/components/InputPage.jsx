...
const InputPage = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      if (inputType === "youtube") {
        const response = await fetch(`${config.NODE_BACKEND}/download-audio`, {
          method: "POST",
        ...
        const uploadResponse = await axios.post(
          `${config.NODE_BACKEND}/api/video/uploadVideo`,
          formData,
          {
        ...
        const transcriptionResponse = await axios.post(
          `${config.NODE_BACKEND}/transcribe`,
        ...
        const saveTranscriptionResponse = await axios.post(
          `${config.NODE_BACKEND}/api/video/uploadTranscription`,
          payload,
          {
            headers: { "Content-Type": "application/json" },
            withCredentials: true,
  ...
  return (
    <div className="bg-black min-h-[50vh] flex flex-col items-center justify-center p-4 pb-20">
      <div
        className="relative w-full max-w-xl mt-40 mb-10 p-6 rounded-2xl shadow-2xl 
      ...
      >
        ...
        <div className="flex items-center mb-3 md:space-x-4 space-x-2">
          <label
            htmlFor="inputType"
            className="md:text-lg text-sm font-semibold text-gray-200"
          >
            Select Input Type:
          </label>
          <select
        ...
        {inputType === "youtube" ? (
          ...
          <div className="py-6 bg-black/60 backdrop-blur-lg rounded-lg shadow-lg">
            ...
            <div
              className="flex flex-col justify-center border-dashed border-gray-500 
